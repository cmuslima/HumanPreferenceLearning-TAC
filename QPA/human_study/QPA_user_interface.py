...
import os
import sys
script_dir  = os.path.dirname(os.path.abspath(__file__))   # .../QPA/human_study
qpa_dir  = os.path.dirname(script_dir)                   # .../QPA
root_dir    = os.path.dirname(qpa_dir)                   # .../root
config_path = os.path.join(root_dir, 'config/train_QPA.yaml')
...
def main(cfg):
    ...
    wandb.init(
        ...
        name=f'testingUI_QPA_{cfg.experiment}_{cfg.env}_{cfg.max_feedback}_{cfg.seed}',
        entity="musliman",
        mode='online',
    )
    workspace.run()
